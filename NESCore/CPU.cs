...
public class CPU
{
    ...
    public void PowerUp()
    {
        ...
        for (ushort i = 0x4000; i <= 0x4013; i++) Bus.WriteByte(i, 0x00);
    ...
    /// </summary>
    private void Ora(AddressingModes mode)
    {
    ...
    private void AdcInternal(byte value)
    {
    ...
    private void Sbc(AddressingModes mode, bool invalid = false)
    {
        var value = GetAddressingModeParameter(mode);
    ...
    private void DeltaMemory(AddressingModes mode, int delta, string mnemonic)
    {
    ...
    private void Eor(AddressingModes mode)
    {
        var value = GetAddressingModeParameter(mode);
    ...
    private void EorInternal(byte value)
    {
        A ^= value;
    ...
    private void SetFlagValue(Flags flag, bool isSet, string mnemonic)
    {
    ...
    private void Clc()
    {
        SetFlagValue(Flags.Carry, false, "CLC");
    }

    private void Sec()
    {
        SetFlagValue(Flags.Carry, true, "SEC");
    }

    private void Cli()
    {
        SetFlagValue(Flags.IRQ, false, "CLI");
    }

    private void Sei()
    {
        SetFlagValue(Flags.IRQ, true, "SEI");
    }

    private void Clv()
    {
        SetFlagValue(Flags.Overflow, false, "CLV");
    }

    private void Cld()
    {
        SetFlagValue(Flags.Decimal, false, "CLD");
    }

    private void Sed()
    {
        SetFlagValue(Flags.Decimal, true, "SED");
    }
    ...
    private void LoadRegister(ref byte register, AddressingModes mode, string mnemonic)
    {
        LogInstruction(mode, mnemonic);
        var value = GetAddressingModeParameter(mode);
    ...
    private byte Lsr(AddressingModes mode)
    {
    ...
    private byte LsrInternal(byte value)
    {
    ...
    private void TransferRegister(byte source, ref byte destination, string mnemonic, bool updateFlags = true)
    {
        LogInstruction(0, mnemonic);
    ...
    private void DeltaRegister(ref byte register, int delta, string mnemonic)
    {
        LogInstruction(0, mnemonic);
        register += (byte) delta;
        Bit.Val(ref P, Flags.Zero, register == 0);
    ...
    private byte Rol(AddressingModes mode)
    {
        var value = GetAddressingModeParameter(mode);
    ...
    private byte Ror(AddressingModes mode)
    {
        var value = GetAddressingModeParameter(mode);
    ...
    private byte Rotate(byte value, RotateDirection direction)
    {
    ...
    /// <param name="mnemonic">
    ///     Opcode mnemonic to log the instruction. If null is provided then this method
    ///     won't log the instruction. It assumes that the caller wants to create a bespoke log for the specific call
    /// </param>
    private void StoreRegister(byte value, AddressingModes mode, [AllowNull] string mnemonic)
    {
        var addr = GetAddressingModeAddress(mode);
        LogInstruction(mode, mnemonic);
        Bus.WriteByte(addr, value);
    ...
    private void Txs()
    {
        TransferRegister(X, ref SP, "TXS", false);
    }

    private void Tsx()
    {
        TransferRegister(SP, ref X, "TSX");
    }
    ...
    private void Aso(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Rla(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Lse(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Rra(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Axs(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Lax(AddressingModes mode)
    {
    ...
    private void Dcm(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    private void Ins(AddressingModes mode)
    {
        var addr = GetAddressingModeAddress(mode);
    ...
    public byte ZPageXParam()
    {
        return Bus.Byte(ZPageX(Bus.Byte(PC + 1)));
    }

    public byte ZPageYParam()
    {
        return Bus.Byte(ZPageY(Bus.Byte(PC + 1)));
    }

    public byte ZPageParam()
    {
        return Bus.Byte(Bus.Byte(PC + 1) & 0x00FF);
    }

    public byte AbsoluteParam()
    {
        return Bus.Byte(Absolute(Bus.Word(PC + 1)));
    }
    ...
    public byte AbsoluteYParam(bool checkPageCrossed = false)
    {
        ...
        if (checkPageCrossed) CheckPageCrossed(parameter, absY);

        return result;
    }

    public byte IndirectXParam()
    {
        return Bus.Byte(IndirectX(Bus.Byte(PC + 1)));
    }
    ...
    {
        if ((addr1 & 0xFF00) != (addr2 & 0xFF00)) cyclesThisSec++;
    }
    ...
    private void LogInstruction(AddressingModes mode, string mnemonic, bool invalid = false)
    {
        ...
        switch (mode)
        {
            ...
            case AddressingModes.ZeroPageX:
                numParams = 1;
                instruction.AppendFormat("${0:X2},X @ {1:X2} = {2:X2}", Bus.Byte(PC + 1), ZPageX(Bus.Byte(PC + 1)),
                    ZPageXParam());
                break;
            case AddressingModes.ZeroPageY:
                numParams = 1;
                instruction.AppendFormat("${0:X2},Y @ {1:X2} = {2:X2}", Bus.Byte(PC + 1), ZPageY(Bus.Byte(PC + 1)),
                    ZPageYParam());
                break;
            case AddressingModes.IndirectX:
                numParams = 1;
            ...
            case AddressingModes.AbsoluteX:
                numParams = 2;
                instruction.AppendFormat("${0:X4},X @ {1:X4} = {2:X2}", Bus.Word(PC + 1), AbsoluteX(Bus.Word(PC + 1)),
                    AbsoluteXParam());
                break;
            case AddressingModes.AbsoluteY:
                numParams = 2;
                ...
                instruction.AppendFormat("${0:X4},Y @ {1:X4} = {2:X2}", Bus.Word(PC + 1), absYInitialAddr,
                    AbsoluteYParam());
                break;
            default:
                Invalid();
    ...
    private void LogInstruction(int numParams, string mnemonic, bool invalid = false)
    {
        ...
        for (var i = 1; i <= numParams; i++) sb.Append($"{Bus.Byte(PC + i):X2} ");
